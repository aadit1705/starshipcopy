...
const PERFS = {
  ...
  , getPerfs = () => {
    ...
    const o = (window.performance || Date).now() - a;
    t = deviceSettings.isTablet || deviceSettings.isIE ? PERFS.PERF_BAD : o < 5 ? PERFS.PERF_HIGH : o < 14 ? PERFS.PERF_GOOD : o < 22 ? PERFS.PERF_LOW : PERFS.PERF_BAD,
    navigator && navigator.connection && "4g" !== navigator.connection.effectiveType && (navigator.connection.effectiveType,
    t = PERFS.PERF_BAD),
    PERF = t;
    const n = t == PERFS.PERF_HIGH ? "PERF_HIGH" : t == PERFS.PERF_GOOD ? "PERF_GOOD" : t == PERFS.PERF_LOW ? "PERF_LOW" : "PERF_BAD";
...
const header = getId("header")
  ...
  , addEventListeners = () => {
    ...
    const a = queryAll("[data-video]")
      , o = e => {
        ...
        let o = e.currentTarget.getAttribute("data-video-type");
        o || (o = "youtube"),
        openModal({
            url: t,
            title: a,
  ...
  , decodeImagesLoaderResize = () => {
    ...
    const t = queryAll("img")
      , a = e => e.addEventListener("load", o, !1)
      , o = () => {
        ++e === t.length && onResize()
    }
    ;
    t.forEach(e => (e => {
        e.complete ? o() : e.decode ? e.decode().then(o).catch( () => a(e)) : a(e)
    }
    )(e))
}
  , decodeImage = (e, t=( () => {}
    ...
    const n = new Image
      , r = () => {
        ...
        e.style.backgroundImage = `url('${n.src}')`,
        t()
    }
      , i = () => {
        n.onload = null,
  ...
  , globalRAF = () => {
    ...
    PERF >= PERFS.PERF_HIGH && parallax && SMOOTH_SCROLLING <= innerHeight) {
        const e = Math.round(Math.abs(SMOOTH_SCROLLING))
          ...
          , o = 1 + Math.max(0, Math.min(e / innerHeight, 1)) * scale;
        if (parallax.style.transform = `translate3d(0,${t}px,0) scale3d(${o}, ${o}, ${o}) rotate3d(0, 0, 0.75, ${a}deg)`,
        opacity) {
...
const limit = header.hasAttribute("data-limit") ? header.getAttribute("data-limit") : .45
  ...
  , addIO = () => {
    ...
    o.forEach(e => gsap.set(e, {
        autoAlpha: 0
    }));
    const n = new IntersectionObserver(t => {
        let o = 0;
        t.forEach( (t, n) => {
            const r = t.boundingClientRect.y
    ...
    ,t);
    o.forEach(e => n.observe(e));
    const r = query("#stats .content-center");
...
const modal = getId("modal")
  ...
  , openModal = ({url: e, title: t, type: a}) => {
    modalState = !0,
    ...
    o.to(queryAll(".section"), 1.25, {
        y: -.25 * innerHeight,
        ease: Quint.easeInOut
    }, 0),
    o.staggerFromTo([modal, modalchild], 1.25, {
        y: gsap.utils.wrap([innerHeight, -.5 * innerHeight])
    }, {
        y: 0,
        ease: Quint.easeInOut
        ...
        autoAlpha: 1,
        ease: Expo.easeOut
    }, 0).add(setVideoPlayer(e, a), .75).restart()
}
  , setVideoPlayer = (e, t) => {
    "youtube" == t ? addYoutubeIframe(e) : addXPost(e)
}
  , addYoutubeIframe = e => {
    let t = query("#modal .video-wrapper");
    if (!t)
        return;
    t.innerHTML = `\n    <div id="player" class="plyr__video-embed">\n      <iframe id="iframe"\n              width="500"\n              height="315"\n              src="https://www.youtube.com/embed/${e}"\n              frameborder="0"></iframe>\n    </div>\n  `;
    const a = new Plyr("#player");
    a.on("ready", e => {
        a.play();
        assign(a.elements.container.style, {
            position: "absolute",
            top: 0,
            left: 0,
            width: "100%",
  ...
  , addXPost = e => {
    const t = query("#modal .video-wrapper");
    if (!t)
        return;
    t.replaceChildren();
    ...
    a.setAttribute("data-media-max-width", "1080"),
    a.innerHTML = `\n    <a href="https://twitter.com/SpaceX/status/${e}?ref_src=twsrc%5Etfw"></a>\n  `,
    t.appendChild(a);
    ...
    innerWidth <= 960) {
        queryAll(".slider").forEach(e => {
            ...
            if (!e.hasAttribute("data-vehicle") && t && t.classList.contains("not-mobile"))
                return;
            const o = e.querySelector(".swiper-button-prev")
              , n = e.querySelector(".swiper-button-next");
            if (o && n && !e.hasAttribute("data-arrow-noresize")) {
                const t = e.querySelector(".js-bounding")
                  , r = e.getBoundingClientRect()
                  , i = t.getBoundingClientRect().top - r.top + 9 + 10;
                o.style.top = n.style.top = `${i}px`,
